...
\begin{equation}
    ...
    q_{\text{SoR}}(\mathbf f_* | \mathbf y) &= \mathcal N \left (
        Q_{*, \mathbf f} (Q_{\mathbf f, \mathbf f} + \sigma^2 I)^{-1} \mathbf y,
        Q_{*, *} - Q_{*, \mathbf f} (Q_{\mathbf f, \mathbf f} + \sigma^2 I)^{-1} Q_{\mathbf f, *}
        \right ) \\
        &= \mathcal N \left (
...
\begin{proof}
    \[
    \begin{aligned}
        (Q_{\mathbf f, \mathbf f} + \sigma^2 I)^{-1} 
        &= (K_{\mathbf{f, u}} K_{\mathbf{u, u}}^{-1} K_{\mathbf{u, f}} + \sigma^2 I)^{-1} \\
        &\rightarrow \text{using matrix inversion lemma} \\
        &= \sigma^{-2} I - \sigma^{-4} K_{\mathbf{f, u}} (K_{\mathbf{u, u}} + \sigma^{-2} K_{\mathbf{u, f}} K_{\mathbf{f, u}})^{-1} K_{\mathbf{u, f}} \\
        &= \sigma^{-2} I - \sigma^{-4} K_{\mathbf{f, u}} \Sigma K_{\mathbf{u, f}} \\
        &\rightarrow \sigma^{-2} K_{\mathbf{u, f}} K_{\mathbf{f, u}} = \Sigma^{-1} - K_{\mathbf{u, u}} \\
        Q_{*, \mathbf f} (Q_{\mathbf f, \mathbf f} + \sigma^2 I)^{-1}
        &= \sigma^{-2} K_{\mathbf{*, u}} K_{\mathbf{u, u}}^{-1} K_{\mathbf{u, f}} - \sigma^{-2} K_{\mathbf{*, u}} K_{\mathbf{u, u}}^{-1} (\Sigma^{-1} - K_{\mathbf{u, u}}) \Sigma K_{\mathbf{u, f}} \\
        &= \sigma^{-2} K_{\mathbf{*, u}} \Sigma K_{\mathbf{u, f}}
    \end{aligned}
    \]
\end{proof}
...
\begin{equation}
    ...
    q_{\text{DTC}}(\mathbf f_* | \mathbf y) &= \mathcal N \left (
        Q_{*, \mathbf f} (Q_{\mathbf f, \mathbf f} + \sigma^2 I)^{-1} \mathbf y,
        K_{*, *} - Q_{*, \mathbf f} (Q_{\mathbf f, \mathbf f} + \sigma^2 I)^{-1} Q_{\mathbf f, *}
        \right ) \\
        &= \mathcal N \left (
...
What is interesting is that we can derive the same result by using a variational approach, first we get the evidence lower bound (ELBO) with $q(u) = \mathcal N(m_u, S_u)$
and $q(f, u) = p(f|u)q(u)$
\begin{equation}
    \begin{aligned}
        \log p(y) &= \log \int p(y, f, u) df du \\
        &= \log \int p(y, f, u) \frac{q(f, u)}{q(f, u)} df du \\
        &= \log \int q(f, u) \frac{p(y, f, u)}{q(f, u)} df du \\
        &= \log \mathbb E_{q(f, u)} \left [ \frac{p(y, f, u)}{q(f, u)} \right ] \\
        &\geq \mathbb E_{q(f, u)} \left [ \log \frac{p(y, f, u)}{q(f, u)} \right ] \quad \text{(Jensen's inequality)} \\
...
\begin{equation}
    \begin{aligned}
        ...
        &= \int p(f|u) q(u) \log p(y|f) df du \quad y|f \sim \mathcal N(f, \sigma^2) \\
        &= \int q(u) \int P(f|u) \log p(y|f) df du \quad f|u \sim \mathcal N(K_{f, u}K_{u, u}^{-1}u, K_{f, f} - Q_{f, f})
    \end{aligned}
\end{equation}

With $\log p(y|f) = -\frac{n}{2} \log 2\pi - n \log \sigma - \frac{1}{2\sigma^2} (y-f)^T(y-f)$, we can get the inner integral
\begin{equation}
    \begin{aligned}
        \int p(f|u) \log p(y|f) df &= -\frac{n}{2} \log 2\pi - n \log \sigma - \frac{1}{2\sigma^2} \mathbb E_{p(f|u)} \left [ (y-f)^T(y-f) \right ] \\
        & \text{Using expectation of quadratic form} \\
        &= -\frac{n}{2} \log 2\pi - n \log \sigma \\
        &- \frac{1}{2\sigma^2} \left [ (y - K_{f, u}K_{u, u}^{-1}u)^T(y - K_{f, u}K_{u, u}^{-1}u) + \text{Tr}(K_{f, f} - Q_{f, f}) \right ] \\
    \end{aligned}
\end{equation}

Then the entire integral is
\begin{equation}
    \begin{aligned}
        \mathbb E_{q(f, u)} \left [ \log p(y|f) \right ] &= -\frac{n}{2} \log 2\pi - n \log \sigma \\
        &- \frac{1}{2\sigma^2} \left [ (y - K_{f, u}K_{u, u}^{-1}m_u)^T(y - K_{f, u}K_{u, u}^{-1}m_u) + \text{Tr}(K_{f, f} - Q_{f, f}) \right ] \\
        &- \frac{1}{2\sigma^2} \text{Tr}(K_{f, u}K_{u, u}^{-1}S_u K_{u, u}^{-1}K_{u, f})
    \end{aligned}
\end{equation}

Second term in ELBO is then
\begin{equation}
    \begin{aligned}
        -\text{KL} \left [q(u) || p(u) \right ] &=
        -\mathbb E_{q(f, u)} \left [ \log q(u) \right ] + \mathbb E_{q(f, u)} \left [ \log p(u) \right ] \\
        &= -\frac{1}{2} \left [ -m + \log |K_{u, u}| - \log |S_u| + \text{Tr}(S_u K_{u, u}^{-1}) + m_u^T K_{u, u}^{-1} \right ]
    \end{aligned}
\end{equation}

Then combine the two parts and get the partial derivatives we can get the optimal $m_u$ and $S_u$
\begin{equation}
    \begin{aligned}
        m_u &= \sigma^{-2} K_{u, u} \Sigma K_{u, f} y \\
        S_u &= K_{u, u} \Sigma K_{u, u}
    \end{aligned}
\end{equation}
where $\Sigma = (K_{u, f} K_{f, u} / \sigma^2 + K_{u, u})^{-1}$.

Then we can get the predictive distribution
\begin{equation}
    \begin{aligned}
        q(f_* | y) &= \int p(f_* | u) q(u) du \\
        &= \mathcal N (K_{*, u} K_{u, u}^{-1} m_u, K_{*, *} - Q_{*, *} + K_{*, u} K_{u, u}^{-1} S_u K_{u, u}^{-1} K_{u, *}) \\
        &= \mathcal N (\sigma^{-2} K_{*, u} \Sigma K_{u, f} y, K_{*, *} - Q_{*, *} + K_{*, u} \Sigma K_{u, *})
    \end{aligned}
\end{equation}

This is because $p(f_* | u) = \mathcal N (K_{*, u} K_{u, u}^{-1} u, K_{*, *} - Q_{*, *})$ and $q(u) = \mathcal N(m_u, S_u)$, so we can use the total expectation and variance formula or the linear transform rule.
In detail
\[
\begin{aligned}
    \mathbb E(f_* | y) &= \mathbb E_{q(u)} [\mathbb E(f_* | u)] = \mathbb E_{q(u)} [K_{*, u} K_{u, u}^{-1} u] = K_{*, u} K_{u, u}^{-1} m_u \\
    \text{Var}(f_* | y) &= \mathbb E_{q(u)} [\text{Var}(f_* | u)] + \text{Var}_{q(u)} [\mathbb E(f_* | u)] \\
    &= \mathbb E_{q(u)} [K_{*, *} - Q_{*, *}] + \text{Var}_{q(u)} [K_{*, u} K_{u, u}^{-1} u] \\
    &= K_{*, *} - Q_{*, *} + K_{*, u} K_{u, u}^{-1} S_u K_{u, u}^{-1} K_{u, *}
\end{aligned}
\]

This is same as the DTC above.

Then I can introduce a recursive update method using the RLS / Kalman-gain form in \textit{Learning For Predictive Control:
A Dual Gaussian Process Approach}, at time $k-1$ we have the posterior $q(u) = \mathcal N(m_u^{k-1}, S_u^{k-1})$, then at time $k$ we have a new data point $(x_k, y_k)$,
$p(y_k|u) = \mathcal N(K_{f_k, u} K_{u, u}^{-1} u, \sigma^2)$, then we can update the posterior
\begin{equation}
    \begin{aligned}
        q(u|y_k) &= \frac{q(y_k|u)q(u)}{p(y_k)} \\
        &\propto q(y_k|u)q(u) \\
        &\rightarrow - \log q(y_k|u)q(u) \quad \text{take the negative log} \\
        &= \frac{1}{2} (u - m_u^{k-1})^T S_u^{k-1} (u - m_u^{k-1}) + \frac{1}{2\sigma^2} (y_k - K_{f_k, u} K_{u, u}^{-1} u)^2 + \text{const} \\
        &\rightarrow \text{keeping only terms that depend on } u \\
        &= \frac{1}{2} u^T (S_u^{k-1} + \sigma^{-2} K_{u, u}^{-1} K_{u, f_k} K_{f_k, u} K_{u, u}^{-1}) u - (S_u^{k-1} m_u^{k-1} + \sigma^{-2} K_{u, u}^{-1} K_{u, f_k} y_k) u + \text{const} \\
        &= \frac{1}{2} u^T A u - b^T u + \text{const} \\
        &= \frac{1}{2} (u - A^{-1} b)^T A (u - A^{-1} b) + \text{const}
    \end{aligned}
\end{equation}

So we can get the updated posterior
\begin{equation}    
    \begin{aligned}
        S_u^k &= A^{-1} =  (S_u^{k-1} + \sigma^{-2} K_{u, u}^{-1} K_{u, f_k} K_{f_k, u} K_{u, u}^{-1})^{-1} \\
        m_u^k &= A^{-1}b = S_u^k (S_u^{k-1} m_u^{k-1} + \sigma^{-2} K_{u, u}^{-1} K_{u, f_k} y_k)
    \end{aligned}   
\end{equation}

Using the matrix inversion lemma, we can rewrite $S_u^k$ as
\begin{equation}
    S_u^k = S_u^{k-1} - S_u^{k-1} K_{u, u}^{-1} K_{u, f_k} (\sigma^2 + K_{f_k, u} K_{u, u}^{-1} S_u^{k-1} K_{u, u}^{-1} K_{u, f_k})^{-1} K_{f_k, u} K_{u, u}^{-1} S_u^{k-1}
\end{equation}
Then we can define the Kalman gain
\begin{equation}
    K_k = S_u^{k-1} K_{u, u}^{-1} K_{u, f_k} (\sigma^2 + K_{f_k, u} K_{u, u}^{-1} S_u^{k-1} K_{u, u}^{-1} K_{u, f_k})^{-1}
\end{equation}
Then the update equations become
\begin{equation}
    \begin{aligned}
        m_u^k &= m_u^{k-1} + K_k (y_k - K_{f_k, u} K_{u, u}^{-1} m_u^{k-1}) \\
        S_u^k &= S_u^{k-1} - K_k K_{f_k, u} K_{u, u}^{-1} S_u^{k-1}
    \end{aligned}
\end{equation}

I can give you the exact formulation in the paper, if we define $\Phi_k = K_{f_k, u} K_{u, u}^{-1}$
\begin{equation}
    \begin{aligned}
        r_k &= y_k - \Phi_k m_u^{k-1} \\
        G_k &= \sigma^2 + \Phi_k S_u^{k-1} \Phi_k^T \\
        L_k &= S_u^{k-1} \Phi_k^T G_k^{-1} \\
        m_u^k &= m_u^{k-1} + L_k r_k \\
        S_u^k &= S_u^{k-1} - L_k G_k L_k^T
    \end{aligned}
\end{equation}

\begin{remark}
    We have to note that the above update method is not good when the new comming data is far away from the inducing points, because the inducing points are fixed and not updated.
\end{remark}
